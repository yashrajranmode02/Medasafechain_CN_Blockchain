...
export default function BatchCard({ title, children, className = "" }) {
  return (
    <motion.div
    ...
    >
      ...
      {/* Inner card */}
      <div className="relative bg-black/80 backdrop-blur-md rounded-2xl p-5 border border-gray-800 h-full flex flex-col">
        {title && (
        ...
        <div className={`text-gray-300 text-sm leading-relaxed ${className}`}>{children}</div>
      </div>
    </motion.div>
  );
}
